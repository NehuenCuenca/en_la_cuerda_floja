<template>
    <div class="bg-beigeStrong min-h-[70vh] p-9 flex flex-col gap-y-8 justify-start items-center">
        ...
        <ul v-else class="flex justify-around items-start gap-8 flex-wrap">
            <li v-for="({name}, i) in categories" :key="i" class=" p-2.5 border-none rounded-tr-lg rounded-bl-lg text-brown-200 text-lg font-semibold bg-brown-900 hover:bg-brown-400 hover:text-brown-900 cursor-pointer transition-colors">{{ name }}</li>
        </ul>
    </div>
</template>
